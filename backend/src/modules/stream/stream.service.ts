...
export class StreamService {
  ...
  async segment(segmentPath: string, res: Response, req: Request): Promise<void> {
    const hlsFolder: string | undefined = req.headers["content-folder"] as string | undefined;
    if (!hlsFolder || typeof hlsFolder !== "string")
      throw new BadRequestException(ResponseMessages.SRC_INVALID);

    const segmentFilePath: string = path.join(path.resolve(), hlsFolder, segmentPath);
